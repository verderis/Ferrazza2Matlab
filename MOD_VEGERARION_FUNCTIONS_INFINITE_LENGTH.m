...
for j = 1:nij %jLoop: DO			  	% j = incidence angular interval index
      ...
      for icode = 1:2 %icodeLoop: DO 
            ...
            for i = 1:nij %iLoop: DO          % i = scattering angular interval index
                ...
                xff(:,:,:,:) = 0.0;						      % scattering function initialization
                ...
                for ialpha = 1:nalpha %alphaLoop: DO 
                      ...
                      for ibeta = 1: nbeta %betaLoop: DO 
                            ...
                            %        end
                                        ...
                                        for ii = 1:nmax
                                            cni = ni;
                                            ju(ii) = besselj(ii,u); %REVISAR!                                            
                                            JB(ii) = besselj(ii,cni); %first kind! 
                                            YB(ii) = bessely(ii,cni); %second kind!
                                            H(ii) = besselh(ii,cni); %third kind!
                                        end
                                        ...
                                        for ii = 1:nmax                                       
                                            hni1(ii) = (ii*besselj(ii, cni))/cni + bessely(ii + 1, cni)*1i - besselj(ii + 1, cni) - (i*bessely(ii, cni)*1i)/cni; % Derivates of Hankel functions
                                            ...
                                            %diff(besselj(nu,z))
                                            ju1(ii) = (i*besselj(ii, u))/cni - besselj(ii + 1, u);
                                        end
                                        %for n = 0:nmax
                                        for n = 1:nmax+1
                                             rr1 = ju1(n)/(u*ju(n));
                                        ...
                                        for k2 = 1:ns %k2Loop: DO      % integration within ith theta_s interval using Gauss - Legendre technique
                                              ...
                                              for k3 = 1:nm1 %k3Loop: DO 
                                                    ...
                                                    tvs = (sin(alpha - phisg)*sgamma + cos(alpha - phisg)*sbeta*cgamma)*cthetas - cbeta*cgamma*sthetas;
                                                    ths = sin(alpha - phisg)*sbeta*cgamma - cos(alpha - phisg)*sgamma;
                                                    cthetsl = (sin(alpha - phisg)*sgamma + cos(alpha - phisg)*sbeta*cgamma)*sthetas + cbeta*cgamma*cthetas;
                                                    ...
                                                    if(beta == 0.d0 && gamma == 0.d0) 
                                                      cphisl = cos(alpha - phisg);
                                                      sphisl =  - sin(alpha - phisg);
                                                    else
                                                      cphisl = (cbeta*sthetas*cos(alpha - phisg) - sbeta*cthetas)/tts;
                                                      sphisl = ((sbeta*sgamma*cos(alpha - phisg) - cgamma*sin(alpha - phisg))*sthetas + cbeta*sgamma*cthetas)/tts;
                                                    end
                                                    ...
                                                    a(1) = (k/(2.0*li))*(zmin - z(1));    %  (38 of [7])
                                                    b(1) = (k/(2.0*li))*(zmin + z(1));
                                                    %
                                                    ...
                                                    for n = 2:nmax0+2
                                                          ...
                                                          ff1vv = ff1vv + svv*cos(n*(phisl - phil));
                                                          ff1vh = ff1vh + svh*sin(n*(phisl - phil));
                                                          ff1hv = ff1hv + shv*sin(n*(phisl - phil));
                                                          ff1hh = ff1hh + shh*cos(n*(phisl - phil));
                                                    end
                                                    % (42) of [7]
                                                    ff1vv = ff1vv*ff;
                                                    ...
                                                    if (icode == 2 && i == j && k2 == k1 && k3 == 1) % forward direction
                                                          fv1fs = real(ff1vv,8)+1i*abs(imag(ff1vv));
                                                          fh1fs = real(ff1hh,8)+1i*abs(imag(ff1hh));
                                                          if(beta == 0.0 && gamma == 0.0)
                                                                fvfs = fv1fs;
                                                                fhfs = fh1fs;
                                                          else
